namespace JustOrderIt.Services.Data
{
        ...
        where T : FileInfo, new()
    {
        ...
        {
            var image = this.Repository.GetById((int)this.IdentifierProvider.DecodeToIntId(id));
            return image;
        }

        public override T GetByEncodedIdFromNotDeleted(string id)
        {
            var image = this.Repository.GetByIdFromNotDeleted((int)this.IdentifierProvider.DecodeToIntId(id));
            return image;
        }
